...
{
	hal->hal_ops->rx_desc_copy_end_tlv(fdesc, ldesc);
}
...
static inline void ath12k_dp_rxdesc_set_msdu_len(struct ath12k_hal *hal,
						 ...
						 u16 len)
{
	hal->hal_ops->rx_desc_set_msdu_len(desc, len);
}
...
static inline void ath12k_dp_rx_desc_get_dot11_hdr(struct ath12k_hal *hal,
						   ...
						   struct ieee80211_hdr *hdr)
{
	hal->hal_ops->rx_desc_get_dot11_hdr(desc, hdr);
}
...
{
	hal->hal_ops->rx_desc_get_crypto_header(desc, crypto_hdr, enctype);
}

static inline u8 ath12k_dp_rx_get_msdu_src_link(struct ath12k_hal *hal,
						struct hal_rx_desc *desc)
{
	return hal->hal_ops->rx_desc_get_msdu_src_link_id(desc);
}
