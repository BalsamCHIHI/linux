				      ...
				      struct sk_buff *skb)
{
	...
	if (pdev_id < 1 || pdev_id > MAX_RADIOS) {
			      ...
			      enum htt_srng_ring_id *htt_ring_id)
{
	...
	switch (ring_type) {
	case HAL_RXDMA_BUF:
		/* for some targets, host fills rx buffer to firmware and firmware fills
		 * to rxbuf ring for each rxdma
		 */
				     ...
				     struct htt_rx_ring_tlv_filter *tlv_filter)
{
	...
	if (tlv_filter->rx_mpdu_start_wmask > 0 &&
		 ...
		 * please assign it in future if any device is
