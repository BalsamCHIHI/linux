...
struct ieee80211_bss_conf *ath12k_get_link_bss_conf(struct ath12k_link_vif *arvif)
{
	...
	struct ieee80211_bss_conf *link_conf;
	/* ieee80211_vif->link_conf[]s are rcu objects which requires rcu_read_lock()
			     ...
			     struct ieee80211_bss_conf *bss_conf)
{
	...
	struct ieee80211_vif *vif = ath12k_ahvif_to_vif(ahvif);
	struct ath12k_wmi_vdev_up_params params = {};
	struct ath12k_wmi_peer_assoc_arg peer_arg = {0};
	struct ieee80211_link_sta *link_sta;
	struct ath12k_link_sta *arsta;
...
void __ath12k_mac_scan_finish(struct ath12k *ar)
{
	...
	case ATH12K_SCAN_STARTING:
		...
		ieee80211_queue_work(ar->ah->hw, &ar->scan.vdev_clean_wk);
		break;
	}
}
...
static void ath12k_scan_vdev_clean_work(struct work_struct *work)
{
	struct ath12k *ar = container_of(work, struct ath12k,
					 scan.vdev_clean_wk);
	 ...
	 * arvif is already deletd. Alternatively, if the scan vdev is not
				 ...
				 struct ieee80211_scan_request *hw_req)
{
	...
	ath12k_dbg(NULL, ATH12K_DBG_MAC,
		   "mac hw scan request received. Frequency range %d - %d MHz",
		   hw_req->req.channels[0]->center_freq,
		   hw_req->req.channels[hw_req->req.n_channels - 1]->center_freq);
					 ...
					 struct ieee80211_vif *vif)
{
	...
	cancel_work_sync(&ar->scan.vdev_clean_wk);
}
					   ...
					   struct ieee80211_vif *vif)
{
	...
	for (link_id = 0; link_id < ATH12K_NUM_MAX_LINKS; link_id++) {
		...
		/* Scanning was in progress, abort it */
...
static void ath12k_mac_setup(struct ath12k *ar)
{
	...
	INIT_WORK(&ar->scan.vdev_clean_wk, ath12k_scan_vdev_clean_work);
...
static int __ath12k_mac_mlo_setup(struct ath12k *ar)
{
	...
	for (i = 0; i < ag->num_devices; i++) {
		...
		for (j = 0; j < partner_ab->num_radios; j++) {
			...
			ath12k_dbg(ab, ATH12K_DBG_MAC,
				   "Device [%d] pdevid %d link id %d num_link %d\n",
				   i, j, pdev->hw_link_id, num_link);
		}
	}

	mlo_params.group_id = cpu_to_le32(ag->id);
...
void ath12k_mac_mlo_teardown(struct ath12k_hw_group *ag)
{
	...
	for (i = 0; i < ag->num_hw; i++) {
		...
		for_each_ar(ah, ar, j) {
			...
			if (ret) {
				ath12k_err(ar->ab, "failed to teardown %d\n", ret);
				break;
			}
		}
	}
...
int ath12k_mac_allocate(struct ath12k_hw_group *ag)
{
	 ...
	 */
	if (ag->mlo_capable & ATH12K_INTER_DEVICE_MLO_SUPPORT)
