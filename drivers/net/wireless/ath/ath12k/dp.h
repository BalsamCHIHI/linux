...
{
	return dp->ops->dp_device_init(dp);
}
...
static inline u32 ath12k_dp_tx_get_vdev_bank_config(struct ath12k_dp *dp,
						    struct ath12k_link_vif *arvif)
{
	return dp->ops->dp_tx_get_vdev_bank_config(dp->ab, arvif);
}
...
static inline int ath12k_dp_arch_reo_cmd_send(struct ath12k_dp *dp,
					      ...
					      void (*cb)(struct ath12k_dp *dp, void *ctx,
							 enum hal_reo_cmd_status status))
{
	return dp->ops->dp_reo_cmd_send(dp->ab, rx_tid, type, cmd, cb);
}
...
static inline void ath12k_dp_arch_rx_peer_tid_delete(struct ath12k_dp *dp,
						     ...
						     u8 tid)
{
	dp->ops->rx_peer_tid_delete(ar, peer, tid);
}
...
static inline int ath12k_dp_arch_peer_rx_tid_reo_update(struct ath12k_dp *dp,
							...
							bool update_ssn)
{
	return dp->ops->peer_rx_tid_reo_update(ar, peer, rx_tid,
...
ath12k_dp_hw_grp_to_dp(struct ath12k_dp_hw_group *hw_grp, u8 device_id)
{
	return hw_grp->dp[device_id];
}
