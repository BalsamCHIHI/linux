...
int ath12k_peer_mlo_create(struct ath12k_hw *ah, struct ieee80211_sta *sta)
{
	...
	if (ml_peer) {
		/* KVALO: fix NULL */
		ath12k_warn(NULL, "ML peer (%d) exists already, unable to add new entry for %pM",
			    ml_peer->id, sta->addr);
		return -EEXIST;
	}
	...
	if (ahsta->ml_peer_id == ATH12K_MLO_PEER_ID_INVALID) {
		/* KVALO: fix NULL */
		ath12k_warn(NULL, "unable to allocate ML peer id for sta %pM", sta->addr);
		kfree(ml_peer);
		return -ENOMEM;
	}
	...
	ath12k_dbg(NULL, ATH12K_DBG_MAC, "ml peer created %pM id %d\n",
...
int ath12k_peer_mlo_delete(struct ath12k_hw *ah, struct ieee80211_sta *sta)
{
	...
	if (!ml_peer) {
		/* KVALO: fix NULL */
		ath12k_warn(NULL, "ML peer for %pM not found", sta->addr);
		return -EINVAL;
	}
	...
	ath12k_dbg(NULL, ATH12K_DBG_MAC, "ML peer deleted %pM\n", sta->addr);
