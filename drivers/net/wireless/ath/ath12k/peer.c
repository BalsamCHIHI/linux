				       ...
				       const u8 *addr, bool expect_mapped)
{
	...
	ret = wait_event_timeout(ab->peer_mapping_wq, ({
		...
		res = !!ath12k_dp_link_peer_find_by_vdev_id_and_addr(dp, vdev_id, addr);
		       ...
		       struct ath12k_wmi_peer_create_arg *arg)
{
	...
	spin_lock_bh(&dp->dp_lock);
	peer = ath12k_dp_link_peer_find_by_pdev_id_and_addr(dp, ar->pdev_idx,
							    arg->peer_addr);
	if (peer) {
		spin_unlock_bh(&dp->dp_lock);
		return -EINVAL;
	...
	ath12k_dp_link_peer_assign(ath12k_ab_to_dp(ar->ab), &(ath12k_ar_to_ah(ar)->dp_hw),
				   arvif->vdev_id, sta ? sta->addr : NULL,
				   (u8 *)arg->peer_addr, link_id, ar->hw_link_id);
...
static int ath12k_link_sta_rhash_addr_tbl_init(struct ath12k_base *ab)
{
	...
	lockdep_assert_held(&ab->sta_rhash_tbl_lock);
...
int ath12k_link_sta_rhash_tbl_init(struct ath12k_base *ab)
{
	...
	mutex_lock(&ab->sta_rhash_tbl_lock);
	ret = ath12k_link_sta_rhash_addr_tbl_init(ab);
	mutex_unlock(&ab->sta_rhash_tbl_lock);
